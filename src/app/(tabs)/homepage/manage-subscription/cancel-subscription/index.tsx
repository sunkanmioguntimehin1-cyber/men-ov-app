...
//     const requestBody = {
//       note: selectedReason,
//       customerId: getUserData?.data?.id,
//     };
...
const CancelSubscription = () => {
  ...
  const cancelSubscription = useCancelSubscriptionApi(handleCancelPress);

  // React.useEffect(() => {
  //   if (cancelSubscription.isSuccess && cancelSubscription.data?.url) {
  //     setPortalUrl(cancelSubscription.data.url);
  //   }
  // }, [cancelSubscription.isSuccess, cancelSubscription.data?.url]);
  ...
  return (
    <Screen className="flex-1 bg-[#F5F5F8]">
      ...
      {/* Bottom Buttons — fixed above keyboard */}
      {getUserData?.data?.billingAction?.startedAt ? null : (
        <View className="absolute bottom-0 left-0 right-0 flex-row gap-4 px-5 pb-20 pt-4 bg-[#F5F5F8]">
          {/* Submit — outlined red */}
          <TouchableOpacity
            className="flex-1 border-2 border-red-500 rounded-2xl items-center justify-center"
