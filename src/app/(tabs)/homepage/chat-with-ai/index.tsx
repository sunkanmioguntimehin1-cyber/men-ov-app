...
//   // ✅ NEW: Refs to prevent duplicate requests
...
//         // App has come to the foreground - don't trigger new requests
//       }
...
//       // ✅ DON'T remove typing indicator yet - keep it until we get first chunk
...
const ChatWithAi = () => {
  ...
  // ✅ Use getAllChatAiHistory instead of getChatHistory
  const getAllChatAiHistory = useGetAllChatAiHistory();
  const getSessionWithAi = useGetSessionWithAiStream(chatMessage);
  const sendMessage = useTalkToChatAi();
  ...
  // ✅ NEW: Transform server data from getAllChatAiHistory to message format
  useEffect(() => {
    let isMounted = true;

    if (getAllChatAiHistory.data?.messages && Array.isArray(getAllChatAiHistory.data.messages)) {
      const serverMessages = getAllChatAiHistory.data.messages;
      
      // Transform server messages to your app's format
      const transformedMessages: Message[] = serverMessages.map((msg: any) => {
        ...
        return {
          id: msg.id,
          text: msg.content,
          isAi: msg.role === "assistant", // ✅ Map "assistant" to AI messages, "user" to user messages
          timestamp: timestamp,
          date: date,
          fullDate: fullDate,
        };
      ...
      // ✅ Sort by creation date (oldest first) to maintain chat order
      transformedMessages.sort((a, b) => 
        a.fullDate.getTime() - b.fullDate.getTime()
      );

      // ✅ Replace store messages with server data
  ...
  // ✅ Use getAllChatAiHistory loading/error states
              ...
              <View className="flex-row items-center space-x-2">
                <View className="flex-1 mx-2">
                  <CustomInput
                    ...
                    autoCapitalize="sentences"
                    editable={
                      !isLoadingHistory && !sendMessage.isPending && !isStreaming
                    }
                  />
                </View>
                <TouchableOpacity
