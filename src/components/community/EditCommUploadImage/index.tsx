...
const EditCommUploadImage = ({ setNotePublicUrls, notePublicUrls }: any) => {
  const [storeData, setStoreData] = React.useState<string | any>(null);
  const [imageSelected, setImageSelected] = React.useState<any>(null);

 

  console.log("imageSelected in CommUploadImage:", imageSelected);
  console.log("storeData in CommUploadImage:", storeData?.publicUrl);

  console.log("notePublicUrls in CommUploadImage:", notePublicUrls);
  ...
  return (
    <ScrollView>
      <View className="my-5 ">
        {imageSelected || notePublicUrls.length > 0 ? (
              ...
              <View className=" flex-row ">
                {ImgIsError ? (
                  ...
                  <View className=" w-full h-80 relative overflow-hidden rounded-2xl ">
                    <Image
                      source={{ uri: storeData?.publicUrl || notePublicUrls }}
...
export default EditCommUploadImage;
